...
class Cache {
    ...
    private var trackCacheOperations: Int = 0
...
let cache = Cache()
var varFiltersCg = [String]()

varFiltersCg.append(cache.add("article-123", "https://coderbyte.com/article-123"))
varFiltersCg.append(cache.add("article-456", "https://coderbyte.com/article-456"))
varFiltersCg.append(cache.add("how-to-code-444", "https://coderbyte.com/how-to-code-444"))
varFiltersCg.append(cache.get("first-article"))
varFiltersCg.append(cache.get("second-article"))
varFiltersCg.append(cache.get("article-456"))
varFiltersCg.append(cache.add("article-123", "https://coderbyte.com/article-123"))
varFiltersCg.append(cache.size())

let output = varFiltersCg.joined(separator: " ")
print(output)


import Foundation

// __define-ocg__ - Age Counting Program
// This program counts items with age >= 50 from JSON data
...
// Split the data string by commas and process each item
let items = dataObject.data.components(separatedBy: ", ")

var output2 = 0

// Count items with age >= 50
for item in items {
    if item.hasPrefix("age=") {
        let ageString = item.replacingOccurrences(of: "age=", with: "")
        if let age = Int(ageString), age >= 50 {
            output2 += 1
        }
    }
}
